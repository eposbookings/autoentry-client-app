...
type RtiWorkflowTask={type:"FPS"|"EPS_NO_PAYMENT"|"EPS_RECOVERY";periodNumber:number;taxMonth:number;reason:string;amount?:number;statutoryPayByType?:Record<string,number>};
type PayrollWorkflowStatus={rti:{count:number;returnCount?:number;periods:number[];tasks:RtiWorkflowTask[]};pensions:{count:number;periods:number[]}};
const emptyPayrollWorkflowStatus:PayrollWorkflowStatus={rti:{count:0,returnCount:0,periods:[],tasks:[]},pensions:{count:0,periods:[]}};
...
const fetchWorkspaceResource=async(url:string,init?:RequestInit)=>{
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{
      const response=await payrollFetch(url,{cache:"no-store",...init});
...
export type PayrollTransport=(input:RequestInfo|URL,init?:RequestInit)=>Promise<Response>;
let payrollTransport:PayrollTransport=(input,init)=>globalThis.fetch(input,init);
export function configurePayrollTransport(transport?:PayrollTransport){
  payrollTransport=transport||(async(input,init)=>globalThis.fetch(input,init));
}
const payrollApiUrl=(input:RequestInfo|URL)=>typeof input==="string"&&input.startsWith("/api/")?`/api/payroll/${input.slice(5)}`:input;
const payrollFetch=(input:RequestInfo|URL,init?:RequestInit)=>payrollTransport(payrollApiUrl(input),init);
...
export const EmployerContext=createContext<EmployerId>(1);
const useEmployerId=()=>useContext(EmployerContext);
export const TaxYearContext=createContext("2026/27");
const useTaxYear=()=>useContext(TaxYearContext);
export const PayFrequencyContext=createContext<PayrollFrequency>("monthly");
const usePayFrequency=()=>useContext(PayFrequencyContext);
export const FirstPayDateContext=createContext("");
...
export default function Home(){
  ...
  async function authenticate(action:"login"|"bootstrap"){
    setError("");
    try{
      const response=await payrollFetch("/api/admin/session",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,employerId:1,email,password,displayName})}),body=await readJsonResponse(response);
  ...
  async function signOut(){
    await payrollFetch("/api/admin/session",{method:"DELETE"});setSession({authenticated:false,setupRequired:false});
  }
...
export function PayrollApp({admin,memberships,activeEmployerId,setActiveEmployerId,signOut}:PayrollAppProps) {
  ...
  async function processPayroll(action:"draft"|"finalise",sourceEmployees=employees,operationSource:"manual"|"pay-details-csv"="manual") {
    ...
    const hasEnteredEmployeePayments=activeEmployees.some(employee=>{
      ...
      return Math.abs(Number(employee.pay||0))>=0.005||Math.abs(Number(employee.hours||0)*Number(employee.rate||0))>=0.005||
        Math.abs(Number(employee.statutoryPayPreview||0))>=0.005||enteredLines.some(item=>Math.abs(Number(item.amount??Number(item.quantity??1)*Number(item.rate??0)))>=0.005);
    });
    ...
    try {
      ...
      const rtiTypes=Array.isArray(result.workflowTasks?.rtiTypes)?result.workflowTasks.rtiTypes:[result.workflowTasks?.rtiType].filter(Boolean);
      const rtiDownstream=result.workflowTasks?.rtiType==="EPS_NO_PAYMENT"
        ...
        :rtiTypes.includes("EPS_RECOVERY")
          ?` RTI now has an FPS task and an EPS statutory-pay recovery task for tax month ${result.workflowTasks.taxMonth}. Both returns are preselected from this payroll period.`
  ...
  async function reopenPayroll() {
    ...
    try {
      const response=await payrollFetch("/api/pay-runs",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({action:"reopen",employerId,taxYear,periodNumber:period})});
  ...
  async function saveEmployeeRecord(employee:Employee) {
    ...
    try {
      const response = await payrollFetch("/api/employees", { method:persistedEmployee?"PUT":"POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  ...
  async function deleteEmployeeRecord(employee:Employee){
    ...
    const response=await payrollFetch("/api/employees",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:employee.id})});
  ...
  async function createPortalInvite() {
    if(!employee)return;
    try {
      const response=await payrollFetch("/api/portal/invites",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,payrollId:employee.payrollId||fallbackPayrollId(employee,taxYear)})});
  ...
  async function downloadPaymentFile() {
    ...
    const response = await payrollFetch("/api/exports", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ employerId,type:"payments",taxYear,periodNumber:period }) });
  ...
  async function downloadPayrollReport(type:string,format:"csv"|"html",filename:string,periodNumber?:number){
    const response=await payrollFetch("/api/reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,type,format,periodNumber})});
  ...
  async function deliverPayslips(method:"email"|"portal"){
    ...
    const response=await payrollFetch("/api/payslip-deliveries",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,periodNumber:period,method})});
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <div className="company"><button type="button" className="return-accountancy" onClick={signOut}>Return to EPOS Accountancy</button><EmployerSwitcher memberships={memberships} activeEmployerId={activeEmployerId} onChange={setActiveEmployerId}/><span>{taxYear}</span></div>
...
type ModulePeriodNotification={label:"FPS"|"EPS";count:number};
type ModulePeriodItem={value:number;prefix:string;label:string;status:string;disabled?:boolean;done?:boolean;title?:string;notifications?:ModulePeriodNotification[];compactStatus?:boolean};
function ModulePeriodBar({title,subtitle,items,value,onSelect,ariaLabel}:{title:string;subtitle:string;items:ModulePeriodItem[];value:number;onSelect:(value:number)=>void;ariaLabel:string}){
  ...
  return <div className="period-wrap module-period-wrap" aria-label={ariaLabel}><div className="period-label"><b>{title}</b><small>{subtitle}</small></div><button type="button" className="period-arrow previous" aria-label={`Show earlier ${subtitle.toLowerCase()}`} disabled={!scrollState.left} onClick={()=>scroll(-1)}>‹</button><div className="periods" ref={trackRef} onScroll={updateScrollState}>{items.map(item=>{const notifications=(item.notifications||[]).filter(notification=>notification.count>0);return <button type="button" key={item.value} data-period-value={item.value} aria-current={item.value===value?"true":undefined} className={`${item.value===value?"current":""} ${item.done?"done":""} ${notifications.length?"has-notifications":""} ${item.compactStatus?"compact-status":""}`} disabled={item.disabled} title={[item.title,item.status,notifications.map(notification=>`${notification.label}: ${notification.count} required`).join(" · ")].filter(Boolean).join(" · ")} onClick={()=>onSelect(item.value)}><span>{item.prefix}</span>{notifications.length>0&&<span className="period-notifications" aria-label={notifications.map(notification=>`${notification.count} ${notification.label} return${notification.count===1?"":"s"} required`).join(", ")}>{notifications.map(notification=><em key={notification.label}><small>{notification.label}</small><strong>{notification.count}</strong></em>)}</span>}<b>{item.label}</b><i>{item.status}</i></button>})}</div><button type="button" className="period-arrow next" aria-label={`Show later ${subtitle.toLowerCase()}`} disabled={!scrollState.right} onClick={()=>scroll(1)}>›</button></div>;
}
...
function ModulePage({ active, employerName, employees, finalised,completed,workflowStatus,canEmployeeWrite,onOpenEmployee, onAddEmployee,onSwitchEmployer,onDataChanged, toast }: { active: string; employerName:string;employees: Employee[];finalised:number[];completed:number[];workflowStatus:PayrollWorkflowStatus;canEmployeeWrite:boolean; onOpenEmployee: (id:number) => void; onAddEmployee: () => void;onSwitchEmployer:(id:number)=>void;onDataChanged:()=>Promise<void>; toast: (s: string) => void }) {
  ...
  const config: Record<string, { title: string; subtitle: string }> = {
    Analysis: { title: "Payroll analysis", subtitle: "See labour cost, deductions and trends across the tax year." },
    Employees: { title: "Employees", subtitle: "Manage employment records, tax details and employee portal access." },
    Employer: { title: "Employer settings", subtitle: "Company, PAYE scheme, payroll defaults and client contacts." },
    HMRC: { title: "HMRC liabilities", subtitle: "PAYE, National Insurance and year-to-date payment position." },
    RTI: { title: "RTI submissions", subtitle: "Prepare and validate payroll information before accredited HMRC transmission." },
    CIS: { title: "Construction Industry Scheme", subtitle: "Manage subcontractors, verification and monthly returns." },
    Pensions: { title: "Workplace pensions", subtitle: "Automatic enrolment, memberships, contributions, letters and reports." },
    Reports: { title: "Payroll reports", subtitle: "Generate statutory forms, payroll summaries and employee documents." },
    Clients: { title: "Clients and agents", subtitle: "Manage employer portfolios, owner access and isolated payroll workspaces." },
    Tools: { title: "Tools and data", subtitle: "Imports, exports, notices, setup, audit logs and payroll file controls." },
  };
...
function EmployerCalendarWorkspace({canWrite,toast}:{canWrite:boolean;toast:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    const response=await payrollFetch(`/api/calendar-days?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`,{cache:"no-store"}),body=await response.json();
  ...
  async function update(item:any,action:"cancel"|"restore"){
    const response=await payrollFetch("/api/calendar-days",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:item.id,action})}),body=await response.json();
...
function MidYearStartWorkspace({toast,employees,completed,finalised,onDataChanged}:{toast:(message:string,success?:boolean)=>void;employees:Employee[];completed:number[];finalised:number[];onDataChanged:()=>Promise<void>}) {
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await payrollFetch(`/api/opening-balances?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`,{cache:"no-store"}),body=await response.json();
  ...
  async function save(employee:Employee){
    ...
    try{
      const response=await payrollFetch("/api/opening-balances",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
...
function DataToolsWorkspace({toast}:{toast:(message:string)=>void}) {
  ...
  async function loadVersions(){
    const response=await payrollFetch(`/api/payroll-versions?employerId=${employerId}`),body=await response.json();
  ...
  async function importEmployeeCsv(file?:File){
    ...
    try{
      ...
      const response=await payrollFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"import-employees",employerId,rows})}),body=await response.json();
  ...
  async function importEmployerCsv(file?:File){
    ...
    try{
      ...
      const response=await payrollFetch("/api/employer",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"import-employers",rows})}),body=await response.json();
  ...
  async function downloadBackup(){
    ...
    try{
      ...
      const response=await payrollFetch(`/api/data?employerId=${employerId}`);
      ...
      const analysisResponse=await payrollFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"analyse-restore",employerId,backup})});
  ...
  async function verifyPayload(backup:any){
    const response=await payrollFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"verify-backup",employerId,backup})}),body=await response.json();
  ...
  async function analyseRestore(file?:File){
    ...
    try{
      const backup=await readBackupFile(file),response=await payrollFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"analyse-restore",employerId,backup})}),body=await response.json();
  ...
  async function restore(){
    ...
    try{
      const response=await payrollFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"restore-backup",employerId,backup:restoreBackup,confirmation:restoreConfirmation,currentFingerprint:restoreAnalysis.currentFingerprint})}),body=await response.json();
  ...
  async function saveVersion(){
    setBusy(true);
    try{
      const response=await payrollFetch("/api/payroll-versions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"save",employerId,label:versionLabel,notes:versionNotes})}),body=await response.json();
  ...
  async function analyseVersion(versionId:number){
    ...
    try{
      const response=await payrollFetch("/api/payroll-versions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"analyse",employerId,versionId})}),body=await response.json();
  ...
  async function restoreVersion(){
    ...
    try{
      const response=await payrollFetch("/api/payroll-versions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"restore",employerId,versionId:versionAnalysis.version.id,confirmation:versionConfirmation,currentFingerprint:versionAnalysis.currentFingerprint})}),body=await response.json();
  ...
  async function archiveVersion(version:any){
    ...
    setBusy(true);
    try{
      const response=await payrollFetch("/api/payroll-versions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"archive",employerId,versionId:version.id})}),body=await response.json();
  ...
  async function copyBenefits(){
    ...
    try{
      ...
      do{
        const response=await payrollFetch("/api/benefits",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"copy-tax-year",employerId,sourceTaxYear:sourceBenefitTaxYear,targetTaxYear:taxYear})}),body=await response.json();
...
function AgentWorkspace({toast}:{toast:(message:string,success?:boolean)=>void}) {
  ...
  async function load(){
    const response=await payrollFetch(`/api/agent?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
  ...
  async function operation(json:any,success:string){
    setBusy(true);
    try{
      const response=await payrollFetch("/api/agent",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,...json})}),body=await response.json();
  ...
  async function invoiceAction(id:number,action:"issue"|"void"){
    ...
    setBusy(true);
    try{
      const response=await payrollFetch("/api/agent",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id,action,reason})}),body=await response.json();
...
function AccessWorkspace({toast,onSwitchEmployer}:{toast:(message:string)=>void;onSwitchEmployer:(id:number)=>void}) {
  ...
  async function load(){
    const [response,requestsResponse,sessionResponse]=await Promise.all([payrollFetch(`/api/admin/users?employerId=${employerId}`),payrollFetch(`/api/employee-requests?employerId=${employerId}`),payrollFetch(`/api/admin/session?employerId=${employerId}`)]);
    ...
    if(sessionResponse.ok){
      ...
      const enriched=await Promise.all(memberships.map(async(client:any)=>{
        try{
          const yearEndResponse=await payrollFetch(`/api/year-end?employerId=${client.employerId}&taxYear=${encodeURIComponent(client.taxYear)}`);
  ...
  async function update(user:any,changes:Record<string,unknown>){
    const response=await payrollFetch("/api/admin/users",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,membershipId:user.membershipId,role:user.role,status:user.status,canViewConfidential:user.canViewConfidential,...changes})}),body=await response.json();
...
function ScenarioWorkspace({toast}:{toast:(message:string,success?:boolean)=>void}) {
  ...
  useEffect(() => {
    payrollFetch("/api/scenarios").then(r => {
  ...
  async function createSample(){
    ...
    try{
      const response=await payrollFetch("/api/scenarios",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"create-isolated-sample",confirmation:"CREATE ISOLATED SAMPLE"})}),body=await readJsonResponse(response);
...
function AnalysisWorkspace({toast}:{toast:(message:string)=>void}) {
  ...
  async function load(){
    const response=await payrollFetch(`/api/analysis?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`,{cache:"no-store"}),body=await response.json();
...
function EmployerWorkspace({toast}:{toast:(message:string)=>void}) {
  ...
  useEffect(()=>{payrollFetch(`/api/employer?employerId=${employerId}`).then(r=>r.json()).then(body=>{if(body.employer)setForm({...defaults,...body.employer,payslipDesign:normalisePayslipDesign(body.employer.payslipDesign)});setWarnings(body.warnings||[]);}).catch(()=>toast("Employer settings could not be loaded."));},[]);
  async function loadDepartments(){const response=await payrollFetch(`/api/departments?employerId=${employerId}`),body=await response.json();if(response.ok)setDepartments(body.departments||[]);}
  ...
  async function saveDepartment(){const response=await payrollFetch("/api/departments",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({...departmentDraft,employerId,id:editingDepartmentId})}),body=await response.json();if(!response.ok)return toast(body.error);setEditingDepartmentId(0);await loadDepartments();toast("Department details updated and audited.");}
  async function removeDepartment(id:number){const response=await payrollFetch("/api/departments",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id})}),body=await response.json();if(!response.ok)return toast(body.error);await loadDepartments();toast("Unused department deleted.");}
...
function CisWorkspace({ toast }: { toast: (s: string,success?:boolean) => void }) {
  ...
  async function loadCis() {
    const [response,employerResponse]=await Promise.all([payrollFetch(`/api/cis?employerId=${employerId}`),payrollFetch(`/api/employer?employerId=${employerId}`)]),body=await response.json(),employerBody=await employerResponse.json();
  ...
  async function loadReturn(){const response=await payrollFetch(`/api/cis?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}&taxMonth=${taxMonth}`),body=await response.json();if(response.ok)setReturnData(body);}
  ...
  async function importCisCsv(file?:File){
    ...
    try{
      ...
      const response=await payrollFetch("/api/cis",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"import-subcontractors",employerId,rows})}),body=await response.json();
  ...
  async function issueStatement(subcontractorId:number,deduction:number){
    const response=await payrollFetch("/api/cis",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"issue-statement",employerId,taxYear,taxMonth,subcontractorId,issueGrossStatement:deduction<=0,deliveryMethod:"download"})});
    const body=await response.json();if(!response.ok)return toast(body.error);
    const statementId=Number(body.submission?.id),documentResponse=await payrollFetch(`/api/cis?employerId=${employerId}&action=statement-document&id=${statementId}&format=html`);
...
function RtiWorkspace({ toast,employees,finalised,migrated,workflowStatus,onDataChanged }: { toast: (s: string,success?:boolean) => void;employees:Employee[];finalised:number[];migrated:number[];workflowStatus:PayrollWorkflowStatus;onDataChanged:()=>Promise<void> }) {
  const employerId=useEmployerId(),taxYear=useTaxYear(),payFrequency=usePayFrequency(),firstPayDate=useFirstPayDate();
  const paySchedule=useMemo(()=>scheduledPayPeriods(taxYear,payFrequency,firstPayDate||undefined),[taxYear,payFrequency,firstPayDate]);
  ...
  const [manualRtiResultOpen,setManualRtiResultOpen]=useState(false);
  ...
  function selectWorkflowTask(task:RtiWorkflowTask){
    if(task.type==="FPS"){
      setSelected("FPS");setSubmissionPeriod(task.periodNumber);
    }else{
      setSelected("EPS");setEpsTaxMonth(task.taxMonth);setEpsNoPayment(task.type==="EPS_NO_PAYMENT");
    }
    setDraft(null);setDeclaration(false);setPreviewOpen(false);
  }
  ...
  const currentWorkflowTasks=workflowStatus.rti.tasks||[];
  const selectedFpsTasks=currentWorkflowTasks.filter(task=>task.type==="FPS"&&task.periodNumber===submissionPeriod);
  const selectedEpsTasks=currentWorkflowTasks.filter(task=>task.taxMonth===epsTaxMonth&&(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY"));
  const selectedNoPaymentTask=currentWorkflowTasks.find(task=>task.type==="EPS_NO_PAYMENT"&&task.taxMonth===selectedPayrollTaxMonth);
  const selectedFpsNotRequired=Boolean(selectedNoPaymentClosure||selectedNoPaymentTask);
  const fpsCardStatus=selectedFpsNotRequired
    ?selectedNoPaymentClosure?"Not required · no-payment EPS accepted":"Not required · no employee payments"
    :selectedFpsTasks.length
      ?`${selectedFpsTasks.length} required · ${latestStatus("FPS","Not prepared")}`
      :latestStatus("FPS",finalised.includes(submissionPeriod)?"No FPS task":"Awaiting payroll");
  const selectedEpsReason=selectedEpsTasks.some(task=>task.type==="EPS_NO_PAYMENT")?"no-payment declaration":"statutory-pay recovery";
  const epsCardStatus=selectedEpsTasks.length
    ?`${selectedEpsTasks.length} required · ${selectedEpsReason}`
    :latestStatus("EPS",allFpsCompleteForMonth(epsTaxMonth)?"No EPS due":"Awaiting payroll");
  const submissions = [
    ["FPS","Full Payment Submission",`Period ${submissionPeriod}`,fpsCardStatus],
    ["EPS","Employer Payment Summary",`Tax month ${epsTaxMonth}`,epsCardStatus],
    ["NVR","NINO verification request","Withdrawn 3 February 2025","HMRC suspended"],
    ["Additional FPS","Additional FPS",`Period ${submissionPeriod} · correction only`,selectedFpsNotRequired?"Not applicable · no FPS required":selectedAcceptedFps?latestStatus("Additional FPS","Not required · correction only"):latestStatus("Additional FPS","Awaiting HMRC baseline")],
    ["EXB","Expenses & benefits",taxYear,latestStatus("EXB","Available")],
  ];
  async function loadHistory(){const response=await payrollFetch(`/api/submissions?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),body=await response.json();if(response.ok){setHistory(body);setHistoryLoaded(true);}}
  useEffect(()=>{loadHistory().catch(()=>toast("RTI submission history could not be loaded."));},[]);
  useEffect(()=>{
    if(!historyLoaded||autoPositioned)return;
    const firstWorkflowTask=(workflowStatus.rti.tasks||[])[0];
    if(firstWorkflowTask){selectWorkflowTask(firstWorkflowTask);setAutoPositioned(true);return;}
  ...
  },[historyLoaded,autoPositioned,history,paySchedule,workflowStatus.rti.tasks]);
  useEffect(()=>{
  ...
  useEffect(()=>{payrollFetch(`/api/employer?employerId=${employerId}`).then(response=>response.json()).then(body=>{if(body.employer)setEmployerIdentity({name:body.employer.name,payeReference:body.employer.payeReference||"Not configured"});}).catch(()=>toast("RTI employer identity could not be loaded."));},[]);
  async function validateSubmission() {
    ...
    if(selected==="FPS"&&selectedFpsNotRequired)return toast(`No FPS is due for period ${submissionPeriod} because no employees were paid. Complete the no-payment EPS for tax month ${selectedPayrollTaxMonth} instead.`);
  ...
  async function prepareFiling() {
    ...
    try {
      const response=await payrollFetch("/api/submissions",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({id:draft.id,employerId,declarationAccepted:declaration})});
      const body=await response.json(); if(!response.ok) throw new Error(body.error);
  ...
  async function recordRtiFilingResult(){
    ...
    const response=await payrollFetch("/api/submissions",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({
    ...
    setManualRtiResultOpen(false);setRtiAcknowledgement("");setRtiResponseCode("");setRtiResponseMessage("");await loadHistory();await onDataChanged();
  ...
  const rtiScheduleBase=paySchedule.map(scheduled=>{
    ...
    const noPaymentEpsTask=(workflowStatus.rti.tasks||[]).find(task=>task.type==="EPS_NO_PAYMENT"&&task.taxMonth===scheduled.taxMonth);
    const rawFpsStatus=fps?.status||(payrollFinalised?"Not prepared":payrollMigrated?"Processed in prior system":"Awaiting payroll");
    const fpsStatus=noPaymentEps?"Not required · no-payment EPS accepted":noPaymentEpsTask?"Not required · no employee payments":rawFpsStatus;
    const fpsRequirementComplete=Boolean(noPaymentEps||noPaymentEpsTask)||rawFpsStatus==="accepted"||payrollMigrated;
  ...
  const rtiSchedule=rtiScheduleBase.map(item=>{
    ...
    const allFpsComplete=monthRows.length>0&&monthRows.every(row=>row.fpsRequirementComplete);
    const requiredEpsTask=(workflowStatus.rti.tasks||[]).find(task=>task.taxMonth===item.taxMonth&&(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY"));
    const monthClosed=noPaymentClosed||(allFpsComplete&&!requiredEpsTask);
    const epsStatus=requiredEpsTask
      ?requiredEpsTask.type==="EPS_NO_PAYMENT"?"Required · no-payment declaration":"Required · statutory-pay recovery"
      :item.epsSubmissionStatus==="accepted"
  ...
  const pendingRtiTasks=workflowStatus.rti.tasks||[];
  const notificationsForTaxMonth=(taxMonth:number):ModulePeriodNotification[]=>{
    const fps=pendingRtiTasks.filter(task=>task.type==="FPS"&&task.taxMonth===taxMonth).length;
    const eps=pendingRtiTasks.filter(task=>(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY")&&task.taxMonth===taxMonth).length;
    return [{label:"FPS",count:fps},{label:"EPS",count:eps}].filter(notification=>notification.count>0) as ModulePeriodNotification[];
  };
  const notificationsForPayrollPeriod=(periodNumber:number,taxMonth:number):ModulePeriodNotification[]=>{
    const fps=pendingRtiTasks.filter(task=>task.type==="FPS"&&task.periodNumber===periodNumber).length;
    const eps=pendingRtiTasks.filter(task=>(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY")&&task.taxMonth===taxMonth).length;
    return [{label:"FPS",count:fps},{label:"EPS",count:eps}].filter(notification=>notification.count>0) as ModulePeriodNotification[];
  };
  const filingOptions=<div className="form-grid form-pad rti-filing-options">
    ...
    {manualRtiResultOpen&&<div className="modal-bg" role="dialog" aria-modal="true" aria-label="Record external RTI result manually"><div className="modal rti-result-modal"><header><div><span className="eyebrow">External filing evidence</span><h2>Record external RTI result</h2><p>Use this only after an accredited adapter or HMRC filing service transmitted the exact package. This imports evidence; PayFlow does not claim transmission.</p></div><button type="button" onClick={()=>setManualRtiResultOpen(false)}>Close</button></header><div className="form-body"><div className="form-grid"><label className="field"><span>Test-ready RTI package</span><select value={rtiResultId} onChange={event=>setRtiResultId(Number(event.target.value))}><option value={0}>Select package…</option>{rtiHistory.filter(item=>["test-ready","submitted"].includes(item.status)&&item.type!=="NVR").map(item=><option key={item.id} value={item.id}>#{item.id} · {item.type}</option>)}</select></label><label className="field"><span>HMRC result</span><select value={rtiOutcome} onChange={event=>setRtiOutcome(event.target.value)}><option value="accepted">Accepted</option><option value="rejected">Rejected</option></select></label><Field label="External submission date and time" value={rtiSubmittedAt} type="datetime-local" onChange={setRtiSubmittedAt}/><Field label="Acknowledgement / correlation reference" value={rtiAcknowledgement} onChange={setRtiAcknowledgement}/><Field label="HMRC response code" value={rtiResponseCode} onChange={setRtiResponseCode}/><Field label={rtiOutcome==="rejected"?"Rejection message":"HMRC response message"} value={rtiResponseMessage} onChange={setRtiResponseMessage}/></div></div><footer><button type="button" onClick={()=>setManualRtiResultOpen(false)}>Cancel</button><button className="primary" disabled={!rtiResultId||rtiAcknowledgement.trim().length<6||!rtiSubmittedAt||(rtiOutcome==="rejected"&&rtiResponseMessage.trim().length<3)} onClick={recordRtiFilingResult}>Record external result</button></footer></div></div>}
  </div>;
  const conciseFpsStatus=(status:string)=>status==="Not required · no employee payments"?"No FPS due":status==="Not required · no-payment EPS accepted"?"EPS accepted":status==="Not prepared"?"FPS due":status==="Processed in prior system"?"Prior system":status;
  const rtiPeriodSelector=selected==="EPS"
    ?<ModulePeriodBar title={taxYear} subtitle="RTI tax months" ariaLabel="RTI tax month" value={epsTaxMonth} items={Array.from({length:12},(_,index)=>{
      const value=index+1,existing=rtiHistory.find(item=>item.type==="EPS"&&Number(submissionPayload(item).periodNumber)===value),fpsComplete=allFpsCompleteForMonth(value);
      const requiredTask=(workflowStatus.rti.tasks||[]).find(task=>task.taxMonth===value&&(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY"));
      const status=requiredTask?(requiredTask.type==="EPS_NO_PAYMENT"?"No-pay EPS due":"Recovery EPS due"):existing?.status==="accepted"?"Accepted":existing?.status||(fpsComplete?"No EPS due":"Awaiting payroll");
      return{value,prefix:`M${value}`,label:months[index],status,done:status==="Accepted"||(fpsComplete&&!requiredTask),notifications:notificationsForTaxMonth(value),compactStatus:true};
    })} onSelect={value=>{const epsTask=pendingRtiTasks.find(task=>task.taxMonth===value&&(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY")),fpsTask=pendingRtiTasks.find(task=>task.taxMonth===value&&task.type==="FPS");if(!epsTask&&fpsTask){selectWorkflowTask(fpsTask);return;}setEpsTaxMonth(value);setEpsNoPayment(epsTask?.type==="EPS_NO_PAYMENT");setDraft(null);setDeclaration(false);}}/>
    :["FPS","Additional FPS"].includes(selected)
      ?<ModulePeriodBar title={taxYear} subtitle={`${payrollFrequencyRule(payFrequency).label} RTI periods`} ariaLabel="RTI payroll period" value={submissionPeriod} items={rtiSchedule.map(item=>({value:item.periodNumber,prefix:`P${item.periodNumber}`,label:payFrequency==="monthly"?months[item.taxMonth-1]:new Intl.DateTimeFormat("en-GB",{day:"2-digit",month:"short",timeZone:"UTC"}).format(new Date(`${item.payDate.iso}T00:00:00Z`)),status:conciseFpsStatus(item.fpsStatus),done:item.fpsRequirementComplete,title:`Pay date ${item.payDate.label}`,notifications:notificationsForPayrollPeriod(item.periodNumber,item.taxMonth),compactStatus:true}))} onSelect={value=>{const scheduleItem=rtiSchedule.find(item=>item.periodNumber===value),fpsTask=pendingRtiTasks.find(task=>task.type==="FPS"&&task.periodNumber===value),epsTask=scheduleItem&&pendingRtiTasks.find(task=>task.taxMonth===scheduleItem.taxMonth&&(task.type==="EPS_NO_PAYMENT"||task.type==="EPS_RECOVERY"));if(!fpsTask&&epsTask){selectWorkflowTask(epsTask);return;}setSubmissionPeriod(value);setDraft(null);setDeclaration(false);}}/>
      :null;
  const selectionOptions={map:(_render:unknown)=>rtiPeriodSelector};
  return <div className="operational-workspace"><div className="subnav" aria-label={selected==="EPS"?"RTI tax month":"RTI payroll period"}>{selectionOptions.map((value:number)=><button key={value} className={activeSubmissionPeriod===value?"active":""} onClick={()=>{if(selected==="EPS")setEpsTaxMonth(value);else setSubmissionPeriod(value);setDraft(null);setDeclaration(false);}}>{selected==="EPS"?`Tax month ${value}`:`Period ${value}`}</button>)}</div><div className="submission-cards">{submissions.map(([code,name,period,status])=><button key={code} className={selected===code?"selected":""} onClick={()=>{setSelected(code);setDraft(null);setDeclaration(false);}}><span>{code}</span><b>{name}</b><small>{period}</small><i>{status}</i></button>)}</div><div className="operation-grid"><section className="operation-card"><div className="card-head"><div><h2>{submissions.find(s=>s[0]===selected)?.[1]}</h2><p>{employerIdentity.name} · PAYE {employerIdentity.payeReference}</p></div><span className={`status ${draft?.status==="validated"?"":"amber"}`}>{selected==="NVR"?"HMRC suspended":draft?.status||"Not generated"}</span></div>{filingOptions}<div className="operation-footer rti-action-footer"><div className="rti-confirmation">{selected!=="NVR"&&<Check text="I confirm this return is complete and correct" checked={declaration} onChange={setDeclaration}/>}</div><div className="rti-action-buttons"><button disabled={selected==="NVR"} onClick={()=>draft?setPreviewOpen(true):toast("Generate the draft first.")}>Preview totals</button><button type="button" onClick={()=>setManualRtiResultOpen(true)}>Record external RTI result manually</button><button disabled={filing||selected==="NVR"} onClick={validateSubmission}>{filing?"Validating…":"Generate & validate"}</button><button className="primary" disabled={selected==="NVR"||filing||!draft||draft.status!=="validated"||!declaration} onClick={prepareFiling}>Prepare filing package</button></div></div></section><aside className="calculation-panel"><span>{selected==="EPS"?"EPS recovery and levy":"Submission totals"}</span>{selected==="EPS"?<><SummaryLine label="Statutory pay recovered" value={recoveries.statutoryPayRecovered||0}/><SummaryLine label="CIS deductions suffered" value={recoveries.cisDeductionsSuffered||0}/><SummaryLine label="Apprenticeship Levy" value={recoveries.apprenticeshipLevy||0} strong highlight/></>:<><SummaryLine label="Employees" value={draft?.employeeCount||0} format="number"/><SummaryLine label="Gross pay" value={totals.grossPay||0}/><SummaryLine label="PAYE tax" value={totals.payeTax||0}/><SummaryLine label="Employee NIC" value={totals.employeeNic||0}/><SummaryLine label="Employer NIC" value={totals.employerNic||0}/><SummaryLine label="Net pay" value={totals.netPay||0} strong highlight/></>}<small>{selected==="NVR"?"Use FPS identity matching for employees whose NINO is unknown.":selected==="EPS"?"Values are reconciled from statutory leave, CIS records and the cumulative levy calculation.":"Displayed totals are for the selected finalised payroll period; FPS year-to-date values also include audited migration openings."}</small></aside></div><section className="operation-card"><div className="card-head"><div><h2>RTI submission schedule</h2><p>FPS is due on or before each pay date processed in PayFlow. Imported periods were processed in the previous payroll system and do not create PayFlow filing tasks.</p></div><span>{finalised.length} PayFlow periods finalised · {migrated.length} imported</span></div><div className="report-table-scroll"><table><thead><tr><th>Payroll period</th><th>Payroll</th><th>Pay date / FPS deadline</th><th>FPS filing state</th><th>EPS tax month / deadline</th><th>EPS filing state</th></tr></thead><tbody>{rtiSchedule.map(item=><tr key={item.periodNumber}><td><button onClick={()=>{if(item.payrollFinalised){setSubmissionPeriod(item.periodNumber);setSelected("FPS");setDraft(null);setDeclaration(false);}}}>Period {item.periodNumber} · {payFrequency==="monthly"?months[item.taxMonth-1]:`week ${item.taxWeekNumber}`}</button></td><td><span className={`status ${item.payrollFinalised||item.payrollMigrated?"":"amber"}`}>{item.payrollFinalised?"Finalised":item.payrollMigrated?"Imported history":"Awaiting payroll"}</span></td><td>{item.payDate.label}</td><td><span className={`status ${item.fpsNeedsAction?"amber":""}`}>{item.fpsStatus}</span>{item.fpsNeedsAction&&<small>External filing evidence required</small>}</td><td>Month {item.taxMonth} · {item.epsDue.label}</td><td>{item.epsStatus}</td></tr>)}</tbody></table></div></section><section className="operation-card"><div className="card-head"><div><h2>RTI submission history</h2><p>Validated, superseded and adapter-ready packages retained for this employer. “Prepared” is not an HMRC submission.</p></div><span>{rtiHistory.length} records</span></div><table><thead><tr><th>ID</th><th>Type</th><th>Status</th><th>Due / pay date</th><th>Prepared</th><th>HMRC submitted</th><th>Correlation</th></tr></thead><tbody>{rtiHistory.slice(0,25).map(item=><tr key={item.id}><td>#{item.id}</td><td><b>{item.type}</b></td><td><span className={`status ${item.status==="invalid"?"amber":""}`}>{item.status}</span></td><td>{formatUkDate(item.dueDate)}</td><td>{formatUkDate(item.preparedAt)}</td><td>{formatUkDate(item.submittedAt,"Not transmitted")}</td><td>{item.correlationId||"Local draft"}</td></tr>)}</tbody></table></section></div>;
}
...
function HmrcWorkspace({ toast,onDataChanged }: { toast: (s: string) => void;onDataChanged:()=>Promise<void> }) {
  ...
  async function loadLiabilities(){
    const response=await payrollFetch(`/api/hmrc-liabilities?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),body=await response.json();
  ...
  async function loadNotices(){
    const [noticeResponse,employeeResponse]=await Promise.all([payrollFetch(`/api/hmrc-notices?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),payrollFetch(`/api/employees?employerId=${employerId}`)]);
  ...
  async function exportRecord(format:"csv"|"html"){
    const response=await payrollFetch("/api/reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,type:"p32",format,periodNumber:taxMonth})});
  ...
  async function createNotice(){
    ...
    const response=await payrollFetch("/api/hmrc-notices",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)}),body=await response.json();
  ...
  async function updateNotice(id:number,action:"apply"|"ignore"){
    const response=await payrollFetch("/api/hmrc-notices",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id,action})}),body=await response.json();
  ...
  async function voidPayment(id:number){
    ...
    const response=await payrollFetch("/api/hmrc-payments",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id,reason})}),body=await response.json();
...
function PensionsWorkspace({ toast,employees,finalised,onDataChanged }: { toast: (s: string,success?:boolean) => void;employees:Employee[];finalised:number[];onDataChanged:()=>Promise<void> }) {
  ...
  async function loadPensions(){
    const response=await payrollFetch(`/api/pensions?employerId=${employerId}`),body=await response.json();
  ...
  async function recordDeclaration(){
    try{
      const response=await payrollFetch("/api/pensions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"record-declaration",employerId,schemeId,declarationDate:declarationFiledDate,reference:declarationReference,confirmed:declarationConfirmed})});
  ...
  async function runAssessment() {
    ...
    try {
      const outcomes=await Promise.all(assessable.map(async person=>{
        const response=await payrollFetch("/api/pensions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,payrollId:person.payrollId,taxYear,periodNumber:assessmentPeriod,assessmentDate})});
        const body=await response.json();
  ...
  async function membershipAction(payrollId:string,action:string){
    ...
    try{
      ...
      const response=await payrollFetch("/api/pensions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,employerId,payrollId,taxYear,periodNumber:assessmentPeriod,assessmentDate:actionDate,optOutNoticeDate,optOutNoticeValid,postponementEnd,postponementNoticeDate})});
  ...
  async function downloadContributions(){
    ...
    const response=await payrollFetch("/api/pensions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"export-contributions",employerId,taxYear,periodNumber:targetPeriod})});
  ...
  async function downloadLetter(payrollId:string,letterType:string){
    const response=await payrollFetch("/api/pensions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"letter",employerId,payrollId,letterType})});
...
function ReportPreviewModal({title,employerName,taxYear,columns,rows,onClose}:{title:string;employerName:string;taxYear:string;columns:string[];rows:unknown[][];onClose:()=>void}) {
  return <div className="modal-bg" role="dialog" aria-modal="true" aria-label={`${title} report preview`}><div className="modal report-preview-modal"><header><div><span className="eyebrow">RECONCILED REPORT PREVIEW</span><h2>{title}</h2><p>{employerName} · {taxYear}</p></div><button type="button" aria-label="Close report preview" onClick={onClose}>×</button></header><div className="form-body"><div className="report-preview-summary"><span><b>{rows.length}</b> rows</span><span><b>{columns.length}</b> columns</span><span><b>Reconciled</b> from finalised payroll</span></div><div className="report-table-scroll"><table><thead><tr>{columns.map(column=><th key={column}>{column}</th>)}</tr></thead><tbody>{rows.slice(0,25).map((row,rowIndex)=><tr key={rowIndex}>{row.map((value,columnIndex)=><td key={`${rowIndex}-${columnIndex}`}>{String(value??"")}</td>)}</tr>)}</tbody></table></div>{rows.length>25&&<small className="report-preview-note">Showing the first 25 of {rows.length} rows. The downloaded report contains every row.</small>}</div><footer><button type="button" onClick={onClose}>Close preview</button></footer></div></div>;
}
+function ReportsWorkspace({ toast,employerName,employees,finalised }: { toast: (s: string,success?:boolean) => void;employerName:string;employees:Employee[];finalised:number[] }) {
  ...
  async function refreshYearEnd(){
    const response=await payrollFetch(`/api/year-end?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),body=await response.json();
    if(response.ok)setYearEnd(body);
  }
  useEffect(()=>{refreshYearEnd();},[]);
  async function rollover(){
    const response=await payrollFetch("/api/year-end",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,action:"rollover"})}),body=await response.json();
  ...
  async function previewReport() {
    try {
      const response=await payrollFetch(`/api/reports?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}&type=${reportTypes[report]}${employeeId?`&employeeId=${employeeId}`:""}${periodNumber?`&periodNumber=${periodNumber}`:""}`);
      const body=await response.json(); if(!response.ok)throw new Error(body.error);
      setPreview({columns:body.columns,rows:body.rows}); setPreviewModalOpen(true);
    } catch(error){toast(error instanceof Error?error.message:"Report preview failed.",false);}
  }
  async function generateReport() {
    const response = await payrollFetch("/api/reports", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ employerId,taxYear,type:reportTypes[report],format,employeeId:employeeId||undefined,periodNumber:periodNumber||undefined }) });
  ...
  async function openPrintView(){
    const response=await payrollFetch("/api/reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
...
function EmployeeHistoryModal({employee,close,toast}:{employee:Employee;close:()=>void;toast:(message:string,success?:boolean)=>void}){
  ...
  useEffect(()=>{
    ...
    payrollFetch(`/api/employee-history?employerId=${employerId}&employeeId=${employee.id}`,{cache:"no-store"})
...
function CalendarModal({ employee, period, close, saved }: { employee:Employee; period:number; close:()=>void; saved:(message:string,event?:any,keepOpen?:boolean)=>void }) {
  ...
  async function loadEvents(){const response=await payrollFetch(`/api/leave?employerId=${employerId}`),body=await response.json();if(response.ok)setEvents(body.filter((event:any)=>event.payrollId===(employee.payrollId||`PAY-${employee.id}-${taxYear.slice(0,4)}`)));}
  useEffect(()=>{
    payrollFetch(`/api/employer?employerId=${employerId}`).then(response=>response.json()).then(body=>setSmallEmployer(Boolean(body.employer?.smallEmployersRelief))).catch(()=>undefined);
    payrollFetch(`/api/calendar-days?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`).then(response=>response.json()).then(body=>setEmployerCalendarDays(body.days||[])).catch(()=>undefined);
  ...
  async function calculateAwe(){
    setSaving(true);
    try{const response=await payrollFetch(`/api/leave?employerId=${employerId}&action=calculate-awe&payrollId=${encodeURIComponent(employee.payrollId||`PAY-${employee.id}-${taxYear.slice(0,4)}`)}&relevantDate=${aweRelevantDate}`),body=await response.json();if(!response.ok)throw new Error(body.error);setAwe(body.averageWeeklyEarnings);setAweBasis({...body,averageWeeklyEarningsSource:"finalised-payroll"});saved(body.warning||`AWE ${money(body.averageWeeklyEarnings)} calculated from ${body.paymentCount} finalised payment${body.paymentCount===1?"":"s"}.`,undefined,true);}catch(error){saved(error instanceof Error?error.message:"AWE could not be calculated.",undefined,true);}finally{setSaving(false);}
  }
  async function cancelEvent(event:any){
    const response=await payrollFetch("/api/leave",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:event.id,action:"cancel",reason:cancellationReasons[event.id]||""})}),body=await response.json();
...
function EmployeeLoanModal({employee,close,saved}:{employee:Employee;close:()=>void;saved:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    setLoading(true);
    try{const response=await payrollFetch(`/api/employee-loans?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();if(!response.ok)throw new Error(body.error);setLoans((body.loans||[]).filter((item:any)=>item.payrollId===employee.payrollId));setHistory(body.history||[]);}
  ...
  async function update(id:number,action:"suspend"|"resume"|"stop"){
    const response=await payrollFetch("/api/employee-loans",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id,action})}),body=await response.json();
...
function CashPayRoundingModal({employee,close,saved}:{employee:Employee;close:()=>void;saved:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await payrollFetch(`/api/pay-rounding?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
  ...
  async function create(){
    setSaving(true);
    try{
      const response=await payrollFetch("/api/pay-rounding",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,payrollId:employee.payrollId,unit})}),body=await response.json();
  ...
  async function update(action:"suspend"|"resume"|"stop"|"change-unit"){
    ...
    try{
      const response=await payrollFetch("/api/pay-rounding",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:setting.id,action,unit})}),body=await response.json();
...
function HolidayFundModal({employee,period,periodLocked,periodStart,close,saved}:{employee:Employee;period:number;periodLocked:boolean;periodStart:string;close:()=>void;saved:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await payrollFetch(`/api/holiday-funds?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
  ...
  async function configure(){
    setBusy(true);
    try{
      const response=await payrollFetch("/api/holiday-funds",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
  ...
  async function changeStatus(action:"suspend"|"restore"){
    ...
    try{
      const response=await payrollFetch("/api/holiday-funds",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:setting.id,action})}),body=await response.json();
...
function PayItemModal({employee,period,history,activeLoans,cashRounding,automaticPayrolledBenefits,automaticClass1Benefits,adjustmentTotals,activeAttachmentOrders,nonAttachableStatutoryPay,previousPayDate,payrollCalculationDate,automaticEnrolmentScheme,hasPensionMembership,close,add,saved}:{employee:Employee;period:number;history:CalculationHistory;activeLoans:any[];cashRounding?:any;automaticPayrolledBenefits:number;automaticClass1Benefits:number;adjustmentTotals:{payeTax:number;employeeNic:number;employerNic:number;studentLoan:number;postgraduateLoan:number};activeAttachmentOrders:any[];nonAttachableStatutoryPay:number;previousPayDate:string;payrollCalculationDate:string;automaticEnrolmentScheme?:any;hasPensionMembership:boolean;close:()=>void;add:(items:PayLine[])=>void;saved:(message:string,success?:boolean)=>void}) {
  ...
  async function save(){
    ...
    setSaving(true);
    try{
      const response=await payrollFetch("/api/recurring-items",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,payrollId:employee.payrollId,taxYear,type,name:name.trim(),amount,taxable,nicable,pensionable,startPeriod:period,endPeriod})});
      const body=await response.json();if(!response.ok)throw new Error(body.error);
...
function ScheduleModal({employee,period,close,saved,refresh}:{employee:Employee;period:number;close:()=>void;saved:(message:string)=>void;refresh:()=>Promise<void>}) {
  ...
  async function load(){const response=await payrollFetch(`/api/recurring-items?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),body=await response.json();setItems(Array.isArray(body)?body.filter(item=>item.payrollId===employee.payrollId):[]);setLoading(false);}
  useEffect(()=>{load();},[]);
  async function stop(item:any){const response=await payrollFetch("/api/recurring-items",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:item.id,action:"stop",endPeriod:period-1})}),body=await response.json();if(!response.ok)return saved(body.error||"The schedule could not be stopped.");await Promise.all([load(),refresh()]);saved(`${item.name} stopped. ${body.removedDraftOccurrences||0} future occurrence(s) removed and ${body.invalidatedDraftRuns||0} affected employee draft(s) cleared; finalised history remains unchanged.`);}
...
function RequestInboxModal({close,saved,refresh}:{close:()=>void;saved:(message:string,success?:boolean)=>void;refresh:()=>Promise<void>}){
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await payrollFetch(`/api/employee-requests?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
  ...
  async function review(item:any,decision:"approved"|"rejected"){
    setBusy(item.id);
    try{
      const response=await payrollFetch("/api/employee-requests",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:item.id,decision,reviewNote:notes[item.id]||""})}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
...
function EmailTemplateSettings({toast}:{toast:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    const response=await payrollFetch(`/api/email-templates?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
  ...
  async function saveTemplate(){
    setBusy(true);
    try{
      const response=await payrollFetch("/api/email-templates",{method:editingId?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...form,employerId,...(editingId?{id:editingId}:{})})}),body=await response.json();
  ...
  async function archive(item:any){
    ...
    setBusy(true);try{
      const response=await payrollFetch("/api/email-templates",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id:item.id,action:"archive"})}),body=await response.json();
...
function EmailPayslipsModal({period,close,saved}:{period:number;close:()=>void;saved:(message:string,success?:boolean)=>void}){
  ...
  useEffect(()=>{payrollFetch(`/api/email-templates?employerId=${employerId}`,{cache:"no-store"}).then(async response=>({response,body:await response.json()})).then(({response,body})=>{
  ...
  async function queue(){
    setBusy(true);try{
      const response=await payrollFetch("/api/payslip-deliveries",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,periodNumber:period,method:"email",...(selectedId?{templateId:selectedId}:{})})}),body=await response.json();
...
function PayslipDeliveryModal({close,saved}:{close:()=>void;saved:(message:string,success?:boolean)=>void}){
  ...
  async function load(){
    setLoading(true);
    try{
      const response=await payrollFetch(`/api/payslip-deliveries?employerId=${employerId}`,{cache:"no-store"}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
  ...
  async function resend(item:any){
    ...
    try{
      const response=await payrollFetch("/api/payslip-deliveries",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,taxYear,periodNumber:item.payload.periodNumber,method:item.payload.method,resend:true})}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
      await load();
...
function BenefitRegisterModal({employee,canWrite,close,saved}:{employee:Employee;canWrite:boolean;close:()=>void;saved:(message:string)=>void}) {
  ...
  async function load(){
    setLoading(true);
    try{const response=await payrollFetch(`/api/benefits?employerId=${employerId}`),body=await response.json();if(!response.ok)throw new Error(body.error);setItems((body as BenefitRegisterItem[]).filter(item=>item.employeeId===employee.id));}
  ...
  async function voidBenefit(id:number){
    ...
    try{
      const response=await payrollFetch("/api/benefits",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({employerId,id,action:"void",reason})}),body=await response.json();
      if(!response.ok)throw new Error(body.error);
