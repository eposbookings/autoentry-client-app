...
import { Building2, CalendarClock, KeyRound, ListChecks, Plus, RefreshCw, Save, Trash2 } from "lucide-react";
...
const FALLBACK_CLIENT_TYPES = [
  ...
  { key: "cis_customer", label: "CIS customer", service_keys: ["bookkeeping", "cis", "self_assessment", "self_assessment_payment", "payment_on_account"] },
...
export default function AdminAccountancySettings() {
  ...
  const [hmrcSettings, setHmrcSettings] = useState({
    gateway_user_id: "",
    gateway_password: "",
    gateway_password_saved: false,
    clear_gateway_password: false,
    agent_services_account_id: "",
    self_assessment_agent_code: "",
    corporation_tax_agent_code: "",
    paye_cis_agent_reference: "",
    environment: "test",
    self_assessment_enabled: false,
    corporation_tax_enabled: false,
    paye_cis_enabled: false,
  });
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get("/admin/accountancy/hmrc-settings"),
    ])
      .then(([serviceRes, typeRes, statutoryRes, hmrcRes]) => {
        ...
        setHmrcSettings((current) => ({ ...current, ...(hmrcRes.data || {}), gateway_password: "", clear_gateway_password: false }));
      })
      .catch((e) => {
  ...
  async function save() {
    try {
      ...
      const hmrcRes = await api.put("/admin/accountancy/hmrc-settings", {
        ...hmrcSettings,
        gateway_password: hmrcSettings.gateway_password || null,
      });
      const nextServices = normaliseServiceList(serviceRes.data.services);
      const nextTypes = normaliseClientTypeList(typeRes.data.client_types, nextServices);
    ...
    <div className="mx-auto max-w-[1500px] space-y-4">
      ...
      <Tabs defaultValue="services" className="space-y-4">
        <TabsList className="grid w-full max-w-5xl grid-cols-4">
          ...
          <TabsTrigger value="hmrc" className="gap-2">
            <KeyRound className="h-4 w-4" /> HMRC filing
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="hmrc" className="space-y-3">
          <section className="rounded-md border border-stone-200 bg-white shadow-sm">
            <div className="border-b border-stone-200 px-4 py-3">
              <h2 className="font-display text-lg font-semibold">Practice HMRC filing configuration</h2>
              <p className="mt-1 text-xs text-stone-500">
                Shared by this practice when authorised staff prepare submissions for clients. Client UTRs and scheme references remain on each client record.
              </p>
            </div>
            <div className="grid gap-4 p-4 lg:grid-cols-2">
              <SettingsField label="Government Gateway user ID" value={hmrcSettings.gateway_user_id} onChange={(value) => setHmrcSettings((current) => ({ ...current, gateway_user_id: value }))} />
              <SettingsField label={hmrcSettings.gateway_password_saved ? "Government Gateway password (saved)" : "Government Gateway password"} type="password" value={hmrcSettings.gateway_password} placeholder={hmrcSettings.gateway_password_saved ? "Leave blank to keep saved password" : "Enter password"} onChange={(value) => setHmrcSettings((current) => ({ ...current, gateway_password: value, clear_gateway_password: false }))} />
              <SettingsField label="Agent services account ID" value={hmrcSettings.agent_services_account_id} onChange={(value) => setHmrcSettings((current) => ({ ...current, agent_services_account_id: value }))} />
              <label className="space-y-1 text-sm">
                <span className="font-semibold text-stone-700">Gateway environment</span>
                <select value={hmrcSettings.environment} onChange={(event) => setHmrcSettings((current) => ({ ...current, environment: event.target.value }))} className="h-10 w-full rounded-md border border-stone-200 bg-white px-3">
                  <option value="test">HMRC test / simulation</option>
            ...
            <div className="border-t border-stone-100">
              <HmrcServiceRow
                ...
                description="SA100 and applicable supplementary pages for authorised individuals."
                enabled={hmrcSettings.self_assessment_enabled}
                referenceLabel="Self Assessment agent code"
                reference={hmrcSettings.self_assessment_agent_code}
                onEnabled={(value) => setHmrcSettings((current) => ({ ...current, self_assessment_enabled: value }))}
                onReference={(value) => setHmrcSettings((current) => ({ ...current, self_assessment_agent_code: value }))}
              />
              <HmrcServiceRow
                title="Corporation Tax"
                description="CT600, computations and iXBRL accounts for authorised companies."
                enabled={hmrcSettings.corporation_tax_enabled}
                referenceLabel="Corporation Tax agent code"
                reference={hmrcSettings.corporation_tax_agent_code}
                onEnabled={(value) => setHmrcSettings((current) => ({ ...current, corporation_tax_enabled: value }))}
                onReference={(value) => setHmrcSettings((current) => ({ ...current, corporation_tax_agent_code: value }))}
              />
              <HmrcServiceRow
                title="PAYE and CIS"
                description="PAYE, CIS and ERS use the linked agent reference/enrolment."
                enabled={hmrcSettings.paye_cis_enabled}
                referenceLabel="PAYE/CIS agent reference"
                reference={hmrcSettings.paye_cis_agent_reference}
                onEnabled={(value) => setHmrcSettings((current) => ({ ...current, paye_cis_enabled: value }))}
                onReference={(value) => setHmrcSettings((current) => ({ ...current, paye_cis_agent_reference: value }))}
              />
            </div>
            <div className="border-t border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900">
              Test mode is the safe default. Enabling a service records configuration readiness only; it does not submit anything to HMRC.
            </div>
          </section>
        </TabsContent>
      </Tabs>
...
function HmrcServiceRow({ title, description, enabled, referenceLabel, reference, onEnabled, onReference }) {
  return (
    <div className="grid gap-3 px-4 py-4 lg:grid-cols-[220px_1fr] lg:items-end">
      ...
      <SettingsField label={referenceLabel} value={reference} onChange={onReference} />
    </div>
  );
}
