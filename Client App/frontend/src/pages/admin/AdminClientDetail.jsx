...
import {
  clearCachedAccountingWorkspace,
  fetchAccountingWorkspace,
  updateCachedAccountingClient,
} from "./accountancy-software/accountingWorkspaceCache";
...
export default function AdminClientDetail() {
  ...
  const [pageTab, setPageTab] = useState(() => {
    ...
    return ["details", "software", "deadlines", "items"].includes(requested) ? requested : "details";
  });
  ...
  const loadProviderConfigs = useCallback(async ({ force = false } = {}) => {
    if (providerConfigsLoading || (externalIntegrationLoaded && !force)) return;
    setProviderConfigsLoading(true);
    try {
      const responses = await Promise.all(
        INTEGRATION_PROVIDERS.map(({ value }) => api.get(`/admin/integrations/${value}/config`))
      );
      setProviderConfigs(Object.fromEntries(
        responses.map(({ data }, index) => [INTEGRATION_PROVIDERS[index].value, data])
      ));
  ...
  }, [externalIntegrationLoaded, providerConfigsLoading]);
  ...
  const loadAccountancySoftware = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      ...
      if (accountingDestinationForClient(client || {}) === "external") {
        await loadProviderConfigs({ force });
      }
    } catch (e) {
      toast.error(formatApiError(e));
    } finally {
  ...
  }, [accountancySoftwareLoaded, accountancySoftwareLoading, client, id, loadProviderConfigs]);
  ...
  useEffect(() => {
    ...
    if (pageTab === "details") params.delete("section");
    else params.set("section", pageTab);
  ...
  useEffect(() => {
    ...
    const connectedProvider = provider || (quickbooks ? "quickbooks" : "");
    const connectionStatus = connection || quickbooks;
    if (connectionStatus === "connected" && sync === "ok") {
      toast.success("Accountancy software connected and lists synced");
      setPageTab("software");
      loadAccountancySoftware({ force: true });
    } else if (connectionStatus === "connected" && sync === "error") {
      toast.warning(params.get("message") || "Connected, but list sync needs another try");
      setPageTab("software");
      loadAccountancySoftware({ force: true });
    } else if (connectionStatus === "error") {
      toast.error(params.get("message") || `${providerLabel(connectedProvider)} connection failed`);
    } else if (connectionStatus === "missing") {
      toast.error("Connection callback was missing required details");
    }
    if (quickbooks || provider || connection) {
      const cleanParams = new URLSearchParams(window.location.search);
      ["quickbooks", "provider", "connection", "sync", "message"].forEach((key) => cleanParams.delete(key));
      cleanParams.set("section", "software");
  ...
  async function saveClient() {
    try {
      ...
      const { data: updatedClient } = await api.put(`/admin/clients/${id}`, {
  ...
  async function deleteClient() {
    try {
      ...
      clearCachedAccountingWorkspace(id);
  ...
  async function refreshNativeAccounting() {
    setIntegrationBusy(true);
    try {
      const data = await fetchAccountingWorkspace(id, { force: true });
  ...
  async function connectAccountingSoftware() {
    const provider = integrationSettings.provider;
    const providerName = providerLabel(integrationSettings.provider);
    const providerConfig = providerConfigs[provider] || {};
    if (providerConfig.enabled === false) {
      toast.error(`${providerName} integration is disabled globally`);
      return;
    }
    if (!providerConfig.configured) {
      toast.error(`${providerName} app credentials are not configured on Global integrations`);
      return;
    }
    setIntegrationBusy(true);
    try {
      await persistIntegrationSettings(false);
      const { data } = await api.get(`/admin/integrations/clients/${id}/${provider}/connect`);
  ...
  async function syncAccountingSoftware() {
    ...
    setIntegrationBusy(true);
    try {
      const { data } = await api.post(`/admin/integrations/clients/${id}/${provider}/sync`);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" variant="outline" onClick={() => nav("/admin/accounting")} className="gap-2" data-testid="back-to-clients">
          <ArrowLeft className="h-4 w-4" /> Back to clients
        </Button>
        {client.native_accounting_enabled && (
          <Button
            type="button"
            variant="outline"
            onClick={() => nav(`/admin/accounting?client_id=${encodeURIComponent(id)}`)}
            className="gap-2"
            data-testid="back-to-accounting-software"
          >
            <BookOpen className="h-4 w-4" /> Back to accounting software
          </Button>
        )}
      </div>
      ...
      {pageTab === "deadlines" ? (
        <div className="space-y-4">
          <section className="flex flex-col gap-3 rounded-md border border-emerald-200 bg-emerald-50/40 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Client workspace</p>
              <h2 className="font-display text-xl font-bold text-stone-900">Deadlines</h2>
              <p className="mt-1 text-sm text-stone-600">Review statutory and service deadlines for this client.</p>
            </div>
            <Button type="button" variant="outline" onClick={() => setPageTab("details")}>Open client settings</Button>
          </section>
          <DeadlinesPanel
            client={client}
            tasks={deadlineTasks}
        ...
        <div className="space-y-4">
          <section className="flex flex-col gap-3 rounded-md border border-emerald-200 bg-emerald-50/40 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Client workspace</p>
              <h2 className="font-display text-xl font-bold text-stone-900">Outstanding items</h2>
              <p className="mt-1 text-sm text-stone-600">Manage purchase and sales document requests for this client.</p>
            </div>
            <Button type="button" variant="outline" onClick={() => setPageTab("details")}>Open client settings</Button>
          </section>
      ...
      <Tabs value={pageTab} onValueChange={setPageTab}>
        <TabsList className="grid w-full max-w-2xl grid-cols-2">
          ...
          <TabsTrigger value="software" className="gap-2"><PlugZap className="h-4 w-4" /> Accountancy software integration</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="software" className="mt-4">
          {accountancySoftwareLoading || !accountancySoftwareLoaded ? (
            ...
            <AccountancySoftwarePanel
              ...
              providerConfigsLoading={providerConfigsLoading}
              ...
              loadExternalConfig={loadProviderConfigs}
            />
          )}
        </TabsContent>
      </Tabs>
...
function AccountancySoftwarePanel({
  ...
  const providerConfig = providerConfigs[settings.provider] || {};
  const globallyDisabled = providerConfig.enabled === false;
  const globallyMissing = !providerConfigsLoading && !providerConfig.configured;
  ...
  function selectProvider(value) {
    setSettings((current) => ({
      ...current,
      provider: value,
      status: value === current.provider ? current.status : "not_connected",
      company_id: value === current.provider ? current.company_id : "",
      company_name: value === current.provider ? current.company_name : "",
      sandbox: value === "quickbooks" ? current.sandbox : false,
    }));
  }
  ...
  return (
    <section className="grid gap-4 xl:grid-cols-[0.95fr_1.25fr]">
      <div className="space-y-4">
        <div className="rounded-md border border-stone-200 bg-white p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <span className="mt-1 block text-xs opacity-80">Connect QuickBooks, Xero, or Sage. Only one package can be active.</span>
            </button>
            <button
              type="button"
              disabled={destinationLocked}
        ...
        {nativeEnabled && (
          ...
          >
            ...
            <div className="mt-3 space-y-3 text-sm text-stone-600">
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" onClick={sync} disabled={busy || settings.status !== "connected"} className="gap-2">
                  <RefreshCw className="h-4 w-4" /> Sync external lists
                </Button>
                <Button type="button" variant="outline" onClick={connect} disabled={busy || providerConfigsLoading || globallyDisabled || globallyMissing} className="gap-2">
        ...
        {globallyMissing && (
          <div className="mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            {providerName} app credentials are not configured yet. Save them on Global integrations, then connect this client.
          </div>
        )}
        {settings.status === "connected" && (
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          ...
          />
          <ReadOnlyIntegrationField label="Connection status" value={statusLabel(settings.status)} />
          <ReadOnlyIntegrationField label="Company / tenant ID" value={settings.company_id || "Assigned during connection"} />
          <ReadOnlyIntegrationField label="Company name" value={settings.company_name || "Assigned during connection"} />
        </div>
        ...
        <div className="mt-4 flex flex-wrap justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" onClick={connect} disabled={busy || providerConfigsLoading || globallyDisabled || globallyMissing} className="gap-2" style={{ background: "var(--brand)" }}>
              <PlugZap className="h-4 w-4" /> {settings.status === "connected" ? "Reconnect" : "Connect"} {providerName}
            </Button>
          </div>
        </div>
...
function ReadOnlyIntegrationField({ label, value }) {
  return (
    <div>
      <Label className="text-xs font-semibold text-stone-600">{label}</Label>
      <div className="mt-1 flex h-9 items-center rounded-md border border-stone-200 bg-stone-50 px-3 text-sm text-stone-700">
        {value}
      </div>
    </div>
  );
