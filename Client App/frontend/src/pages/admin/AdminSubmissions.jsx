...
import { DEFAULT_PAGE_SIZE, PaginationFooter, calculateVatInclusiveAmounts, normalisePageSize, pageSlice } from "./accountancy-software/shared";
...
function SalesInvoiceReviewForm({ row, draft, setDraft, activeField, setActiveField, suggestLinesFromPattern, createCustomerFromDraft, codingOptions, codingContext, busy }) {
  ...
  const set = (key, value) => {
    setDraft((current) => {
      ...
      if (key !== "total" && key !== "vat_code") return next;
      const calculated = calculateVatInclusiveAmounts(
        key === "total" ? value : current.total,
  ...
  useEffect(() => {
    setDraft((current) => (
      vatActive
        ? prefillMissingVatCodesFromAmounts(current, options.vatOptions)
        : normalizeSubmittedItemOutsideVatPeriod(current)
    ));
  }, [vatActive, options.vatOptions, setDraft]);
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2.5">
        ...
        <div className="grid gap-2 lg:grid-cols-3">
          ...
          <SearchableLabeledOptionField id="default_location_id" label="Location" value={draft.default_location_id || ""} options={options.locationOptions || []} onChange={(v) => set("default_location_id", v)} activeField={activeField} setActiveField={setActiveField} placeholder="Search CODE - Location" />
          <SearchableLabeledOptionField id="default_dimension_id" label="Dimension" value={draft.default_dimension_id || ""} options={options.dimensionOptions || []} onChange={(v) => set("default_dimension_id", v)} activeField={activeField} setActiveField={setActiveField} placeholder="Search Type: CODE - Dimension" />
        </div>
            ...
            <table className="w-full min-w-[840px] text-xs">
              <thead className="border-b border-stone-200 text-left text-[10px] uppercase tracking-wider text-stone-500">
                <tr>
                  ...
                  <th className="py-1 pr-1.5">Location</th>
                  <th className="py-1 pr-1.5">Dimension</th>
              ...
              <tbody>
                {draft.line_items.map((line, index) => (
                  <tr key={index}>
                    ...
                    {["description", "sales_nominal", "location_id", "dimension_id", "vat_code", "quantity", "unit_price", "net", "vat", "total"].map((key) => (
                      <td key={key} className="py-0.5 pr-1.5">
                        {key === "vat_code" && hasVatOptions ? (
                          ...
                          <SearchableLabeledLineOption value={line[key] || ""} options={options.locationOptions || []} placeholder="Inherit header" onChange={(value) => setLine(index, key, value)} onFocus={() => setActiveField(`line_items.${index}.${key}`)} active={activeField === `line_items.${index}.${key}`} />
                        ) : key === "dimension_id" ? (
                          <SearchableLabeledLineOption value={line[key] || ""} options={options.dimensionOptions || []} placeholder="Inherit header" onChange={(value) => setLine(index, key, value)} onFocus={() => setActiveField(`line_items.${index}.${key}`)} active={activeField === `line_items.${index}.${key}`} />
                        ) : key === "vat_code" ? (
                          <Input
                            value={line[key] || ""}
                            readOnly
...
function ReviewForm({ draft, setDraft, activeField, setActiveField, suggestLinesFromPattern, createSupplierFromDraft, codingContext, codingOptions, busy }) {
  ...
  useEffect(() => {
    setDraft((current) => (
      vatActive
        ? prefillMissingVatCodesFromAmounts(current, codingOptions.vatOptions)
        : normalizeSubmittedItemOutsideVatPeriod(current)
    ));
  }, [vatActive, codingOptions.vatOptions, setDraft]);
  const [patternLineIndex, setPatternLineIndex] = useState(0);
              ...
              <tbody>
                {draft.line_items.map((line, index) => (
                  <tr key={index}>
                    ...
                    {["description", "category", "location_id", "dimension_id", "vat_code", "units", "price", "net", "vat", "total"].map((key) => (
                      <td key={key} className="py-0.5 pr-1.5">
...
function SearchableLabeledOptionField({ id, label, value, options = [], onChange, activeField, setActiveField, placeholder }) {
  const listId = `${id}-labeled-options`;
  ...
  return <div>
    ...
    <Input
      ...
      onChange={(event) => {
        ...
        setText(next);
        const match = options.find((option) => option.label === next || option.value === next);
        onChange(match?.value || (next === "" ? "" : value));
      }}
      className={`mt-0.5 h-8 px-2 text-sm ${active ? "border-emerald-500 ring-2 ring-emerald-100" : ""}`}
    />
...
function SearchableLabeledLineOption({ value, options = [], placeholder, onChange, onFocus, active }) {
...
function buildCodingContextOptions(context = {}) {
  ...
  const locationOptions = uniqueOptions((context.locations || []).filter(activeRecord).map((record) => ({
    value: record.id || record.location_id || "",
    label: [record.code, record.name].filter(Boolean).join(" - "),
  })));
  const dimensionOptions = uniqueOptions((context.dimensions || []).filter(activeRecord).map((record) => ({
    value: record.id || record.dimension_id || "",
    label: `${record.dimension_type_name || "Dimension"}: ${[record.code, record.name].filter(Boolean).join(" - ")}`,
  })));
...
function vatOption(record = {}, native = false) {
  ...
  return {
    ...
    percentage: record.percentage ?? record.vat_rate ?? record.tax_rate ?? record.rate,
  };
}
...
function vatOptionForAmounts(options = [], netValue = "", vatValue = "") {
  const net = Number(netValue);
  const vat = Number(vatValue);
  if (!Number.isFinite(net) || !Number.isFinite(vat) || net === 0 || vat === 0) return "";
  const expectedRate = Math.abs((vat / net) * 100);
  let best = null;
  options.forEach((option) => {
    const match = `${option.value || ""} ${option.label || ""}`.match(/(\d+(?:\.\d+)?)\s*%/);
    if (!match) return;
    const difference = Math.abs(Number(match[1]) - expectedRate);
    if (!best || difference < best.difference) best = { value: option.value, difference };
  });
  return best && best.difference <= 0.35 ? best.value : "";
}

function prefillMissingVatCodesFromAmounts(draft = {}, options = []) {
  const headerVatCode = draft.vat_code || vatOptionForAmounts(options, draft.net, draft.vat);
  let changed = headerVatCode !== (draft.vat_code || "");
  const lines = (draft.line_items || []).map((line) => {
    const lineVatCode = line.vat_code || vatOptionForAmounts(options, line.net, line.vat) || headerVatCode;
    if (lineVatCode === (line.vat_code || "")) return line;
    changed = true;
    return { ...line, vat_code: lineVatCode };
  });
  return changed ? { ...draft, vat_code: headerVatCode, line_items: lines } : draft;
}
