...
export function readSessionState(storageKey, fallback = {}) {
...
export function writeSessionState(storageKey, value) {
  if (!storageKey || typeof window === "undefined") return;
  try {
    window.sessionStorage.setItem(storageKey, JSON.stringify(value));
  } catch {
    // Navigation persistence is a convenience; storage restrictions must not break the workspace.
  }
}

export function vatPercentageForCode(value, options = []) {
  const raw = String(value || "").trim();
  if (!raw) return null;
  const normalised = raw.toLowerCase();
  const option = options.find((candidate) => {
    const candidateValue = String(candidate?.value ?? candidate?.code ?? candidate?.vat_code ?? "").trim().toLowerCase();
    const candidateLabel = String(candidate?.label ?? candidate?.description ?? candidate?.name ?? "").trim().toLowerCase();
    return candidateValue === normalised
      || candidateLabel === normalised
      || (candidateValue && normalised.startsWith(`${candidateValue} -`));
  });
  const explicitRate = option?.percentage ?? option?.vat_rate ?? option?.tax_rate ?? option?.rate;
  if (explicitRate !== undefined && explicitRate !== null && explicitRate !== "") {
    const parsed = Number(String(explicitRate).replace("%", ""));
    if (Number.isFinite(parsed)) return parsed;
  }
  const text = [raw, option?.value, option?.code, option?.label, option?.description, option?.name]
    .filter(Boolean)
    .join(" ");
  const percentageMatch = text.match(/(-?\d+(?:\.\d+)?)\s*%/);
  if (percentageMatch) return Number(percentageMatch[1]);
  if (/\b(no\s*vat|zero(?:\s*rate)?|exempt|outside\s*scope|not\s*applicable)\b/i.test(text)) return 0;
  return null;
}

export function calculateVatInclusiveAmounts(grossValue, vatCode, options = []) {
  if (grossValue === "" || grossValue === null || grossValue === undefined) {
    return null;
  }
  const gross = Number(String(grossValue).replace(/,/g, ""));
  const percentage = vatPercentageForCode(vatCode, options);
  if (!Number.isFinite(gross) || percentage === null || !Number.isFinite(percentage) || percentage <= -100) return null;
  const vat = percentage === 0 ? 0 : gross * percentage / (100 + percentage);
  return {
    gross: grossValue,
...
export function AccountCodeSelect({ accounts, value, onChange, label, placeholder = "Select account" }) {
