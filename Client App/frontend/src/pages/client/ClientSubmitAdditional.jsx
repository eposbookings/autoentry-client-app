...
export default function ClientSubmitAdditional() {
  ...
  function pickFile(f) {
    ...
    setDescription((current) => current.trim() ? current : descriptionFromFilename(f.name));
    setPrefillHint("Prefilled from the document name. Please sense-check it before submitting.");
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        />
        {prefillHint ? (
          <p className="mt-2 text-sm text-stone-500" data-testid="additional-description-prefill-hint">
            {prefillHint}
          </p>
        ) : null}
      </div>
...
function descriptionFromFilename(filename) {
  ...
  return withoutExtension.replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim() || "Additional invoice";
}
