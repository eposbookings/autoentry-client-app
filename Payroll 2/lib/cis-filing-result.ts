export type CisFilingOutcome="accepted"|"rejected";

export function validateCisFilingResult(input:{
  ...
  const errors:string[]=[];
  if(!["test-ready","submitted"].includes(input.currentStatus))errors.push("Only a test-ready or submitted CIS300 package can receive an external filing result.");
  ...
  return {valid:errors.length===0,errors,outcome:input.outcome as CisFilingOutcome,submittedAt:input.submittedAt,acknowledgementReference:reference};
}
