...
function PeriodBar({ period, taxYear, periods, schedule, frequency, onSelect }: { period: number; taxYear:string;periods:PersistedPeriod[];schedule:ScheduledPayPeriod[];frequency:PayrollFrequency; onSelect: (n: number) => void }) {
  ...
  return <div className="period-wrap payroll-period-wrap"><div className="period-label"><b>{taxYear}</b><small>{rule.label} payroll</small></div><button type="button" className="period-arrow previous" aria-label="Show earlier payroll periods" disabled={!scrollState.left} onClick={()=>scrollPeriods(-1)}>‹</button><div className="periods" ref={trackRef} onScroll={updateScrollState}>
    {schedule.map(item => { const n=item.periodNumber,locked=n>open&&!completed.includes(n),date=new Date(`${item.payDate}T00:00:00Z`),shortDate=new Intl.DateTimeFormat("en-GB",{day:"2-digit",month:"short",timeZone:"UTC"}).format(date); return <button key={n} aria-current={n===period?"true":undefined} disabled={locked} onClick={() => onSelect(n)} className={`${n === period ? "current" : ""} ${n === open ? "open" : ""} ${locked ? "future" : ""} ${finalised.includes(n) ? "done" : ""} ${migrated.includes(n)?"migrated":""}`} title={`${formatUkDate(item.periodStart)} to ${formatUkDate(item.periodEnd)}`}>
...
function RtiWorkspace({ toast,employees,finalised,migrated,onDataChanged }: { toast: (s: string,success?:boolean) => void;employees:Employee[];finalised:number[];migrated:number[];onDataChanged:()=>Promise<void> }) {
  ...
  const [draft,setDraft]=useState<{id:number;status:string;totals:Record<string,number>;recoveries:Record<string,number>;employeeCount:number}|null>(null);
  const [history,setHistory]=useState<Array<{id:number;type:string;status:string;dueDate?:string;preparedAt?:string;submittedAt?:string;correlationId?:string;irMark?:string;response?:string;payloadChecksum?:string;payload?:string}>>([]);
  ...
  async function loadHistory(){const response=await payrollFetch(`/api/submissions?employerId=${employerId}&taxYear=${encodeURIComponent(taxYear)}`),body=await response.json();if(response.ok)setHistory(body);}
  useEffect(()=>{loadHistory().catch(()=>toast("RTI submission history could not be loaded."));},[]);
  useEffect(()=>{
    const existing=latestSubmission(selected);
    if(!existing||!["validated","test-ready","accepted"].includes(existing.status)){setDraft(null);setDeclaration(false);return;}
    const payload:any=submissionPayload(existing);
    setDraft({id:existing.id,status:existing.status,totals:payload.totals||{},recoveries:payload.recoveries||{},employeeCount:payload.employees?.length||payload.benefits?.length||(payload.employee?1:0)});
  ...
  async function validateSubmission() {
    ...
    try {
      ...
      setDraft({id:body.submission.id,status:body.submission.status,totals:body.payload.totals||{},recoveries:body.payload.recoveries||{},employeeCount:body.payload.employees?.length||body.payload.benefits?.length||(body.payload.employee?1:0)});
      await loadHistory();
