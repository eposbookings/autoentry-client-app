...
for /f "usebackq delims=" %%S in (`powershell -NoProfile -Command "$bytes=New-Object byte[] 32; $rng=[Security.Cryptography.RandomNumberGenerator]::Create(); $rng.GetBytes($bytes); $rng.Dispose(); -join ($bytes ^| ForEach-Object {$_.ToString('x2')})"`) do set "PAYROLL_INTEGRATION_SECRET=%%S"
